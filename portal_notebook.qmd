...
<!--% Connectez-vous à la base de données, object `con`. Attention, il s'agit d'une base données SQL au format SQLite. -->
...
<!--% Explorez les tables dans cette base de données. Listez les tables. Utilisez des fonctions provenant du package {DBI} chargé précédement. -->
...
<!--% Complétez le code ci-dessous pour lister les champs de chaque table. C'est simlilaire aux colonnes d'un tableau.  -->
...
Les clés primaires et étrangères dans notre modèle de données sont définies. Les clés primaires sont présentées dans le graphiques ci-dessous.
...
Les clés secondaires sont présentées dans le graphiques ci-dessous.
...
<!--% En partant de `rodent_dm2`, effectuez une extraction de la table correspondant au choix indiqué ci-dessus. Créez un tableau unique à partir du résultat mais ne collectez PAS encore les données finales. Nommez cet objet `rodent_red`. Le filtre doit donc limiter les espèces puis les années et enfin le type de plot. -->
...
<!--# Notez bien ceci : jusqu'ici les données sont toujours dans la base et n'ont pas été extraites. Les sorties que vous voyez concernent uniquement les premiers enregistrements à titre d'illustration de ce que la requête va extraire. Par conséquent, R est incapable de déterminer le nombre de lignes que le tableau final aura et l'indique avec "?? x 13" car il sait quand même que ce tableau aura 13 colonnes. Ce n'est qu'en utilisant `collect()` ou `collect_dtx()` que l'on va extraire l'ensemble des données issues du filtre et matérialiser le tableau final. C'est très pratique car la mise au point de la requête se fait plus vite avec une prévisualisation à chaque fois du début du résultat (donc une prévisualisation rapide même si le travail ensuite est conséquent). -->
...
<!-- Le tableau que vous avez obtenu nommé `rodent_sum` doit être remanié pour obtenir un tableau de type "espèces x stations" où chaque ligne correspond à une station et chaque colonne à une espèce. Faites cette transformation et placez-là dans `rodent_wider`. Plusieurs valeurs manquantes vont être générées. Remplacez les valeurs par des 0 en utilisant l'argument values_fill = 0.  Affichez-le ensuite en utilisant `tabularise()`. -->
...
```{r mds2chart, record='RNCS', arg='no.data'}
___  +
...
```{r portaldiscu_comment, output='asis'}
select_answer(r"-{
[] -   L’utilisation d’une base de données relationnelle permet de structurer efficacement des informations complexes issues de plusieurs tables, d’effectuer des filtres ciblés et de ne manipuler que les données pertinentes sans charger l’ensemble du jeu dans la mémoire.
[] -   L’utilisation d’une base de données relationnelle est peu utile dans ce type d’analyse et complexifie inutilement la manipulation des données.
[] -   Une base de données relationnelle empêche de combiner correctement les informations provenant de plusieurs tables.

[] -   La MDS non métrique révèle des regroupements partiels entre types de parcelles, indiquant que certains traitements modifient la composition spécifique des rongeurs.
[] -   La MDS non métrique ne montre aucune structuration liée au type de parcelle, ce qui suggère que les exclos n’affectent pas la répartition des rongeurs.
[] -   La MDS non métrique montre une séparation complète entre tous les types de traitements, ce qui indique que chaque type de parcelle produit un assemblage de rongeurs totalement distinct.}-")
```
